...
Listado.prototype.buscarRestaurante = function(id) {
    let result = this.restaurantes.find((restaurante)=> restaurante.id === id);
    if(result){
        return result;
    }
...
Listado.prototype.obtenerCiudades = function() {
    ...
    var ciudadesSinRepetirse = this.eliminarDuplicados(ciudades);
...
Listado.prototype.obtenerCiudadSinRepetirse = function() {
    var repetir = this.restaurantes.map((restaurante)=> restaurante.rubro);
    var repetirCiudad = this.eliminarDuplicados(repetir);

    return repetirCiudad.sort();
}
...
Listado.prototype.obtenerHorarios = function() {
    ...
    var arregloHorarios = this.restaurantes.map((restaurante)=> restaurante.horarios);
    ...
    var horarios2 = this.eliminarDuplicados(this.horarios);

    return horarios2.sort();
}

Listado.prototype.eliminarDuplicados = function(arrays){
    let result = arrays.filter(function(elem, index, self) {
 ...
 * 
 * @param filtroRubro string 
 * @param filtroCiudad string
 * @param filtroHorario array de string
 */
